...
public class ComputerDAOMySQLTest {
  ...
  public void getNComputersTest() {
    ComputerList companyList = computerDAOMySQL.getNComputers(0, 10);
    assertFalse(companyList == null);
    assertTrue(companyList.size() == 10);
    assertTrue(companyList.get(5).getId() == 5 + 1);
    
    companyList = computerDAOMySQL.getNComputers(-5, 10);
    assertTrue(companyList == null);
    
    companyList = computerDAOMySQL.getNComputers(0, -10);
    assertTrue(companyList == null);
  ...
  public void getNComputersPatternTest() {
    ComputerList companyList = computerDAOMySQL.getNComputers("apple", 10, 10);
    assertFalse(companyList == null);
    assertTrue(companyList.size() == 3);
    assertTrue(companyList.get(1).getId() == 298);
    
    companyList = computerDAOMySQL.getNComputers(null, 0, 10);
    assertFalse(companyList == null);
    assertTrue(companyList.size() == 10);
    assertTrue(companyList.get(5).getId() == 5 + 1);
  }
  ...
  public void dellsTest() {
    long[] ids = new long[2];
    Computer computer1 = new Computer("toto", null, null, null);
    ids[0] = computerDAOMySQL.add(computer1);
    
    computer1 = new Computer("toto", null, null, null);
    ids[1] = computerDAOMySQL.add(computer1);

    computerDAOMySQL.dels(ids);
    assertFalse(computerDAOMySQL.findById(ids[1]).isPresent());
    assertFalse(computerDAOMySQL.findById(ids[0]).isPresent());
  }
